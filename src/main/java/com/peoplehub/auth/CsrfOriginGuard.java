...
/**
 * The cross-site request checks on the cookie-authenticated endpoints (b2-3, B2-3/11).
 *
 * <ul>
 *   <li>Origin (login, refresh, logout): a request that carries an {@code Origin} header must come
 *       from the configured app origin. A request without one (a non-browser client) passes this
 *       check; the CSRF token is the main defence.
...
class CsrfOriginGuard {
    ...
    void requireAllowedOrigin(HttpServletRequest request) {
